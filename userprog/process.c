...
process_execute (const char *file_name) 
{
  ...
  char *save_ptr;
  fn_copy2 = strtok_r((char *) fn_copy2, " ", &save_ptr);
  //mycode1 ends
  ...
  if (success==true)
    {
      ...
      thread_current()->child->load = 1;
      sema_up(&thread_current()->child->forload);
    }
  if (success==false)
    {
      thread_current()->child->load = -1;
...
{
  struct childstruct* c = retrieve_child(child_tid);
  if (c==NULL)
    return -1;
...
load (const char *file_name, void (**eip) (void), void **esp,char **save_ptr) 
{
  ...
  char * fn_copy3 = malloc (strlen(file_name)+1);
  strlcpy(fn_copy3, file_name, strlen(file_name)+1);
  char * save_ptr2;
  fn_copy3 = strtok_r(fn_copy3," ",&save_ptr2);

  file = filesys_open (fn_copy3);
...
setup_stack (void **esp,const char *file_name, char **save_ptr) 
{
  ...
  if (kpage != NULL) 
   ...
   char **argv = malloc(argc_new*sizeof(char *));
   argv[argc_new--] = 0;
